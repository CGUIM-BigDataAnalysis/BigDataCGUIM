...
## 子集Subset - 二維資料 取row & column

資料框物件名稱.`loc[row,column]`
...
-   二維資料
    ...
    -   row and col: 資料框物件名稱`.loc[row,column]`
...
再取出欄位後，加上`.str`，再加上預計套用的功能。

文字取代 `資料框變數[欄位名稱].str.replace(舊值,新值)` **substitute**
...
提示：`pd.to_numeric(資料框變數.欄位)`可一次將整個欄位轉換成數字。
...
-   `資料框變數[”欄位名“].str.find(搜尋字串)`，回傳index，回傳-1表示沒有
-   `資料框變數[”欄位名“].str.count(計算字串)`，回傳個數
-   `資料框變數[”欄位名“].str.contains(搜尋字串)`，回傳True or False
...
-   `filter(r.search,想找的list/想找的資料框欄位)`
...
`pd`為pandas import時取的縮寫名稱，`df1`, `df2`為兩個要結合的資料框變數

## 資料組合 - pd.concat()

`pd`.`concat([df1, df2])`，預設為row方向的合併。
...
`pd`.`concat([df1, df2],axis=1)`，利用`axis=1`改為column方向的合併。
...
可以用`pd`.`merge()`函數將資料框結合，使用方法為`pd.merge(資料框left,資料框right,how=["left","right","inner","outer"],on="結合依據欄位")`

`pd`為pandas import時取的縮寫名稱，`資料框left`, `資料框right`為兩個要結合的資料框變數。
...
`pd.merge(資料框left,資料框right,how=["left","right","inner","outer"],on="結合依據欄位")`
...
inner join - 預設:`pd.merge(資料框left,資料框right,how="inner",on="結合欄位")`
...
`pd.merge(資料框left,資料框right,how="outer",on="結合欄位")`
...
left join: `pd.merge(資料框left,資料框right,how="left",on="結合欄位")`

`pd`為pandas import時取的縮寫名稱，`資料框left`, `資料框right`為兩個要結合的資料框變數。

## 資料結合 (Join) - left
`pd.merge(資料框left,資料框right,how="left",on="結合欄位")`
```{python}
#| echo: true
...
-   依照Province/State,Country/Region,Lat,Long四個欄位，將兩張表格結合，**只留下有對應到的資料**
    -   多個欄位：`on`參數設定維多個欄位組成的list，如`on=['Province/State','Country/Region','Lat','Long']` 
-   請問結合後的資料有幾列？
...
-   `pandas` 套件提供完整的轉換功能
-   `pd`.`pivot()`，`pd`為pandas import時取的縮寫名稱
...
-   `資料框變數.pivot(index=識別欄位, columns=轉寬表欄位名稱, values=轉寬表欄位內容)`

## 長表轉寬表 資料框.pivot()
範例：以`patient`為基準，`obs`欄位的值轉換為新欄位，並將`score`欄位填入
```{python}
#| echo: true
...
## 寬表轉長表 pd.melt()
-   使用`pd.melt(資料匡變數,各種參數)`將寬表轉為長表
-   保留team欄位，設定參數`id_vars='team'`
-   消滅'points', 'assists', 'rebounds'，設定參數`value_vars=['points', 'assists', 'rebounds']`，若為全部非id的欄位，可以省略
-   將其他欄位的名稱整合至metric欄位，設定參數`var_name='metric'`
-   數值整合至amount欄位，設定參數`value_name='amount'`
...
-   長轉寬 `資料框變數.pivot(index=識別欄位, columns=轉寬表欄位名稱, values=轉寬表欄位內容)`
-   寬轉長 `pd.melt(資料框變數, id_vars=識別欄位, value_vars=要轉成長表的欄位,var_name=新增欄位名稱, value_name=新增內容名稱)`
...
-   最簡單的方法是將有缺值的資料移除 `資料框變數.dropna()`

## 遺漏值處理 df.dropna()

若資料型態為資料框，可使用`資料框變數.dropna()`選出完整的資料
