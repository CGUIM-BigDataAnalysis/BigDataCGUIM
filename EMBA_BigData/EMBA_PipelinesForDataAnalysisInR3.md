...
- `tidyr` package提供完整的轉換功能
    - 寬表轉長表 `pivot_longer(資料框/寬表,id.vars=需要保留的欄位)`
    - 長表轉寬表 `pivot_wider(資料框/長表,寬表分列依據~分欄位依據)`
...
airqualityLong<-
  pivot_longer(airquality,
               cols = c("Ozone","Solar.R","Wind","Temp")) ##欄位需要保留"Month","Day"
```
...
- `airqualityLong`資料框中，剩下Month, Day, variable, value等四個欄位 (Column)，屬於長表
