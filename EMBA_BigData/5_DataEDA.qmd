...
-   計算集中趨勢 Central Tendency
    -   平均值 Mean `資料框欄位.mean()` or `st.mean(資料框欄位)`
    -   中位數 Median `資料框欄位.median()` or `st.median(資料框欄位)`

`st`為`import statistics as st`取名的套件縮寫
...
-   The pandas series’s function can deal with multiple columns
    -   得到多個欄位的資料可用`data[[column 1,column 2]]`，`data`為資料框名稱
...
-   最小值 Min `資料框欄位.min()`
-   最大值 Max `資料框欄位.max()`
-   四分位差 Quartiles `資料框欄位.quantile()`
-   變異數 Variance `st.var(資料框欄位)` or `pandas資料框欄位.var()`
-   標準差 Standard deviation `st.stdev(資料框欄位)` or `pandas資料框欄位.std()`
...
-   `pandas資料框欄位.quantile(percentiles you want to get)`
...
-   更新資料框，使用row做子集的方式`資料框變數[row slice]`，將第一個row刪除
...
-   試著統計平均**總價元**
-   試著統計平均**單價元平方公尺**
-   Hint: 資料型態需要調整(`pd.to_numeric(pandas資料框欄位)`)
...
-   列聯表 Crosstabs
    -   `pd.crosstab(pandas資料框欄位1,pandas資料框欄位2)`，`pd`為套件縮寫
-   相關性 Correlation
    -   `pandas資料框欄位.corr(另一個pandas資料框欄位)`
...
-   `pandas資料框.groupby(by=分組依據欄位)`設定分組依據
-   計算各**鄉鎮市區**（以鄉鎮市區作為分組依據）的**交易筆數**、**總價元**
-   `pandas資料框欄位.mean()`可知平均，`pandas資料框.size()`可知資料筆數
