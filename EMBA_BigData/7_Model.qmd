...
## 載入資料
從`datasets`套件中，透過`datasets.load_breast_cancer()`讀取乳癌資料。
...
使用`pd.concat([X_train,y_train], axis = 1)` 左右整合`X_train`和`y_train`兩個資料框，這兩個資料框內容分別為分類標籤（答案）`y_train`以及分類用特徵`X_train`。
...
-   使用`setup(有標籤的訓練資料框,targe=標籤欄位名稱)` 設定 PyCaret AutoML環境
-   `compare_models()` 選擇最好的模型，另存成模型物件`best_model`
...
-   `predict_model(訓練出來的模型物件,data = 測試資料)` 將訓練好的模型`best_model`用在測試集`X_test`中，並儲存成預測結果變數`y_pred_pycaret`
-   `metrics.accuracy_score(正確答案,預測答案)`計算準確率，預測答案可從前一個功能的預測結果變數取出（在`prediction_label`欄位中）
    -   `metrics`來自`from sklearn import metrics`
```{python}
#| echo: true
...
-   `setup(有標籤的訓練資料,targe=標籤名稱)` 設定 PyCaret AutoML環境。有標籤的資料為`train`，預測是否存活的標籤名稱為`Survived`
-   `compare_models()` 選擇最好的模型，並儲存成`best_model`模型物件。
...
-   `evaluate_model(模型物件)` 評估`best_model`模型內容
...
-   `predict_model(訓練出來的模型,data = 測試資料)` 將訓練好的模型`best_model`用在測試集`test`中
...
-   使用過去的3日移動平均到90日移動平均股價，訓練模型，預測單一股票股價會漲還是會跌
-   載入預測用資料集
...
## Hands-on AutoML 開始建模
其他的就交給你們自己嘗試了....步驟：

1.    setup(有標籤的訓練資料,targe=標籤名稱) 設定 PyCaret AutoML環境
2.    compare_models() 選擇最好的模型
3.    evaluate_model(模型物件) 評估模型內容
4.    predict_model(訓練出來的模型,data = 測試資料) 將訓練好的模型用在測試集中
5.    metrics.accuracy_score(正確答案,預測答案)計算準確率
...
-   使用`keras`內建MNIST資料，`keras.datasets.mnist.load_data()`
-   總共有10個分類
-   資料維度(28, 28, 1)
-   區分訓練集與測試集
...
將0\~9透過`keras.utils.to_categorical(資料標籤,幾種預測結果)`轉換為二維的分類答案
...
一層卷積層`layers.Conv2D`，一層池化層`layers.MaxPooling2D`，再一層卷積層`layers.Conv2D`，再一層池化層`layers.MaxPooling2D`，最後全連接層`layers.Flatten`，以及`layers.Dropout`。存成`model`變數
...
## 模型訓練
`batch_size`設定每一批次訓練的資料筆數，`epochs`設定要訓練幾輪，最後使用剛剛設計好的模型架構`model`，執行`model.compile`以及`model.fit`
```{python}
#| echo: true
#| cache: true
batch_size = 128
...
## 模型驗證
使用`model.evaluate(測試資料集,測試資料正確答案)`驗證模型準確度
```{python}
#| echo: true
#| eval: false
#| cache: true
...
```{python}
#| echo: true
test = NVDA[NVDA["Date"]>'2022-01-01'] #2022年後的是測試組
train = NVDA[:len(NVDA)-len(test)] #其他的是訓練組
...
使用keras內的`Sequential()`功能創建新的回歸訓練模型。並增加一層LSTM。最後`regressor.compile`決定模型樣貌。
...
使用`regressor.predict(測試資料)`預測股價。因為資料有標準化，因此用`sc.inverse_transform`還原
